...
def parse_md(path):
    ...
    chapter=int(path.stem); title=f"Chương {chapter}"; body=text
    m=re.match(r"^---\s*\n(.*?)\n---\s*\n",text,re.S)
    if m:
...
def build(src,dst):
    ...
    files=sorted([p for p in src.glob("*.md") if p.stem.isdigit()],key=lambda p:int(p.stem))
    if not files: raise RuntimeError(f"Không có .md trong {src}")
    dst.mkdir(parents=True,exist_ok=True)
    for i,p in enumerate(files,1):
        d=parse_md(p)
        (dst/f"{d['chapter']:04d}.json").write_text(json.dumps(d,ensure_ascii=False,separators=(",",":")),encoding="utf-8")
...
def main():
    ...
    print("Build Part 1..."); n1=build(x.part1,data/"part1")
    print("Build Part 2..."); n2=build(x.part2,data/"part2")
    data.mkdir(parents=True,exist_ok=True)
    idx={"title":"Phàm Nhân Tu Tiên","parts":[
      {"id":"part1","name":"Phần 1 - Phàm Nhân Tu Tiên","chapters":n1},
      {"id":"part2","name":"Phần 2 - Tiên Giới Thiên","chapters":n2}]}
    (data/"index.json").write_text(json.dumps(idx,ensure_ascii=False,indent=2),encoding="utf-8")
    print(f"DONE: Part 1={n1}, Part 2={n2}")
    print("Web:",x.web.resolve())
